...
From start to finish the code in the 2018 original sample ran in 2.27 seconds. Initially this automated return may seem fast but it is notable that it is a very small subset of 12 tickers with a small requested output (3 fields). 
Investment decisions require quick turnaround analysis. If a client wanted to see an unfiltered list of stock exchange tickers (ex: NYSE) and requirs additional output for investment decisions, a nested loop may not be optimal. 
